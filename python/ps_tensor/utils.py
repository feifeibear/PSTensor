...
def torch_to_ps_tensor(t) -> cxx.Tensor:
    return cxx.Tensor.from_dlpack(torch_dlpack.to_dlpack(t))


def tf_to_ps_tensor(t) -> cxx.Tensor:
    return cxx.Tensor.from_dlpack(tf_dlpack.to_dlpack(t))


def ps_tensor_to_torch(t) -> torch.Tensor:
    return torch_dlpack.from_dlpack(t.to_dlpack())


def ps_tensor_to_tf(t) -> tf.Tensor:
    return tf_dlpack.from_dlpack(t.to_dlpack())
